...
export default function AdminHealthWidget() {
    ...
    if (loading) return <div className="h-48 bg-slate-700 rounded-xl animate-pulse" />;

    if (!stats) return <div className="text-red-500">Error cargando salud del sistema</div>;

    return (
        <div className="bg-slate-800 border border-slate-600 rounded-xl shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="p-4 bg-slate-900 rounded-lg border border-slate-700">
                    ...
                    <div className={`text-2xl font-mono font-bold ${stats.balance === null ? 'text-slate-500' :
                            stats.balance < 5 ? 'text-red-400' : 'text-green-400'
                        }`}>
                ...
                <div className="p-4 bg-slate-900 rounded-lg border border-slate-700">
                    ...
                    <div className={`text-2xl font-mono font-bold ${stats.errors24h > 0 ? 'text-amber-600' : 'text-green-400'
                        }`}>
                        {stats.errors24h}
                    </div>
                </div>
            ...
            {/* Error Log */}
            <div>
                <h4 className="text-xs font-semibold text-slate-400 uppercase mb-3">Errores Recientes</h4>
                {stats.recentErrors.length === 0 ? (
                    <div className="flex items-center gap-2 text-sm text-green-400 bg-green-500/20 p-2 rounded">
                        <CheckCircle className="w-4 h-4" /> Todo funciona correctamente
                    </div>
                ) : (
                    <ul className="space-y-2">
                        {stats.recentErrors.map((err: any, i: number) => (
                            <li key={i} className="text-xs bg-red-500/20 text-red-400 p-2 rounded flex justify-between items-center">
